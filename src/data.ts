...
export const rawData: string = `Order Number,Product,Price,Date,Payment Method
...
export const parsedData: SalesRecord[] = rawData
  .trim()
  .split("\n")
  .slice(1)
  .map((row) => {
    const [orderNumber, product, priceStr, date, paymentMethod] = row.split(",");
    return {
      orderNumber,
      product,
